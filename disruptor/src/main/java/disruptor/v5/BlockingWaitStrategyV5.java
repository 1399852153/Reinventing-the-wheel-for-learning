...
public class BlockingWaitStrategyV5 {
    private final Lock lock = new ReentrantLock();
    private final Condition processorNotifyCondition = lock.newCondition();

    public long waitFor(long currentConsumeSequence, SequenceV5 currentProducerSequence,
                        List<SequenceV5> dependentSequences) throws InterruptedException {
        ...
        if(!dependentSequences.isEmpty()){
            ...
            while ((availableSequence = SequenceUtilV5.getMinimumSequence(dependentSequences)) < currentConsumeSequence) {
