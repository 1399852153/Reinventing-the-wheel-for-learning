...
public class OrderEventWorkerConsumer implements MyEventConsumer<OrderModel> {

    private Stack<Integer> priceOrderStack = new Stack<>();
    private int maxConsumeTime;
    private boolean notOrdered = false;

    public OrderEventWorkerConsumer(int maxConsumeTime) {
    ...
    public void consume(OrderModel event, long sequence, boolean endOfBatch) {
        ...
        LogUtil.logWithThreadName("OrderEventWorkerConsumer 消费订单事件：sequence=" + sequence + "     " + event);

        if(this.priceOrderStack.size() == this.maxConsumeTime){
            if(this.notOrdered){
                LogUtil.logWithThreadName("OrderEventWorkerConsumer 消费订单事件失败");
            }else{
                LogUtil.logWithThreadName("OrderEventWorkerConsumer 消费订单事件完毕" + priceOrderStack);
            }
        }
    }
